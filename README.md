...
```bash
## 수정하려는 helm app 이 dailyfeed-member 일 경우
cd dailyfeed-app-helm
cd member
source uninstall-helm.sh ## dailyfeed-member 제거
source install-helm-local.sh ## dailyfeed-member 실행 
```
...
```bash
# 새로운 환경에서 클론 (clone)
git clone --recurse-submodules http://github.com/alpha3002025/dailyfeed-member-svc
cd dailyfeed-member-svc
